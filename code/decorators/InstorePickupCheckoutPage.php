...
class InstorePickupCheckoutPage_Controller extends DataExtension {
	...
	public function StoreLocations($request){
		...
		if(Director::is_ajax()){
			...
			if($region && $region->exists()){
				$stores = InstorePickup::get()->filter(array('ShopConfigID' => $shopConfig->ID, 'RegionID' => $region->ID, 'Published' => 1));
			} else {
				$stores = InstorePickup::get()->filter(array('ShopConfigID' => $shopConfig->ID, 'Published' => 1));
			}
	...
	public function StorePickupFields($storeID = 0){
		if($storeID != 0){
			$storePickup = InstorePickup::get()->filter(array('ID' => $storeID, 'Published' => 1))->first();
